...
def _fmt_usd_compact(v: float) -> str:
    """$x.xx M for millions, $x.xx K for thousands, else full."""
    abs_v = abs(v)
    if abs_v >= 1_000_000:
...
def _render_week(conn, ws: date, we: date, month_idx: int = 0):
    ...
    selected_statuses = st.multiselect(
        "Filter by status:", options=all_statuses, default=[],
        key=filter_key, placeholder="All statuses shown",
    )
    ...
    totals = {
        "Client": "TOTAL", "Bill Code": "",
        "Payroll Hrs": round(full_df["payroll_hrs"].sum(), 2),
    ...
    selected = st.selectbox("Add comment for:", ["-- select a client --"] + list(client_options), key=sel_key)
    ...
    with st.expander("Manage Clients"):
        add_tab, remove_tab, restore_tab = st.tabs(["Add Client", "Remove Client", "Restore Client"])
        ...
        with add_tab:
            with st.form(key=f"add_client_{month_idx}_{ws}_{we}"):
                ...
                new_rem  = st.text_input("Remittance Name (optional)")
        ...
        with remove_tab:
            ...
            if active.empty:
                ...
                if sel and sel != "-- select --":
                    ...
                    st.warning(f"**{sel}** will be hidden from **{ws}** onwards. All prior data is preserved.")
...
def _build_heatmap(df) -> go.Figure:
    ...
    fig = go.Figure(go.Heatmap(
        z=z, x=month_labels, y=clients, text=text,
        texttemplate="%{text}",
        hovertemplate="<b>%{y}</b><br>%{x}: %{text}<extra></extra>",
        colorscale=[[0.0,"#a12c7b"],[0.5,"#d19900"],[0.75,"#edb336"],[0.95,"#437a22"],[1.0,"#01696f"]],
        zmin=0, zmax=100,
        colorbar=dict(title=dict(text="Collection %", font=dict(color="#cdccca")),
                      ticksuffix="%", tickfont=dict(color="#cdccca")),
        xgap=2, ygap=2,
    ))
    fig.update_layout(
        title=dict(text="Collection Rate by Client x Month", font=dict(color="#cdccca", size=15)),
...
def _render_summary(conn):
    ...
    st.markdown("---")
    st.subheader("Collection Rate by Client x Month")
    ...
    for i, month_name in enumerate(available_months):
        with top_tabs[i + 1]:
            ...
            if not weeks:
                ...
                continue
            week_tabs = st.tabs([_fmt_week(ws, we) for ws, we in weeks])
