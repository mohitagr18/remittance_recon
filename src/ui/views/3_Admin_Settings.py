...
st.markdown(
    ...
    <div style='margin-bottom:1.2rem;'>
        <h1 style='margin:0;font-size:1.5rem;font-weight:700;color:#e8eaf0;'>⚙️ Admin Settings</h1>
        <div style='font-size:0.82rem;color:#8892a4;margin-top:4px;'>
            Manage payroll ↔ remittance name mappings, copay clients, and analyst profiles.
        </div>
    </div>
    """,
...
tab_nm, tab_copay, tab_analysts = st.tabs(["📋 Name Match Table", "💊 Copay List", "👥 Analyst Management"])
...
# ═══════════════════════════════════════════════════════════════════════════
# TAB 3: Analyst Management
# ═══════════════════════════════════════════════════════════════════════════
with tab_analysts:
    st.markdown("<div class='section-header'><h3>👥 Analyst Roster</h3></div>", unsafe_allow_html=True)
    
    # Handle Inline Edits
    if "analysts_editor" in st.session_state:
        edits = st.session_state["analysts_editor"].get("edited_rows", {})
        if edits:
            analysts_df = conn.execute("SELECT id FROM analysts ORDER BY id").df()
            for str_id, changes in edits.items():
                row_idx = int(str_id)
                db_id = int(analysts_df.iloc[row_idx]["id"])
                
                if "name" in changes:
                    conn.execute("UPDATE analysts SET name = ? WHERE id = ?", [changes["name"], db_id])
                if "is_active" in changes:
                    conn.execute("UPDATE analysts SET is_active = ? WHERE id = ?", [changes["is_active"], db_id])
            
            conn.commit()
            st.success("Analyst edits saved successfully.")
            del st.session_state["analysts_editor"]
            st.rerun()

    # Fetch and Display
    analysts_df = conn.execute("SELECT id, name, is_active, created_at FROM analysts ORDER BY id").df()
    ...
    if not analysts_df.empty:
        st.data_editor(
            ...
            column_config={
                "id": st.column_config.NumberColumn("ID", width="small"),
                "name": st.column_config.TextColumn("Analyst Name", required=True),
                "is_active": st.column_config.CheckboxColumn("Active"),
                "created_at": st.column_config.DatetimeColumn("Created At"),
            }
        )
    else:
        st.info("No analysts found. Add one below.")

    st.markdown("<div class='section-header'><h3>➕ Add Analyst</h3></div>", unsafe_allow_html=True)
    with st.form("add_analyst_form", clear_on_submit=True):
        new_analyst_name = st.text_input("Analyst Name (First Last)", placeholder="e.g. Jane Doe")
        if st.form_submit_button("Add Analyst"):
            if not new_analyst_name.strip():
                st.error("Analyst Name is required.")
            else:
                try:
                    conn.execute("INSERT INTO analysts (id, name) VALUES (nextval('seq_analysts'), ?)", [new_analyst_name.strip()])
                    conn.commit()
                    st.success(f"Added {new_analyst_name.strip()} successfully.")
                    st.rerun()
                except Exception as e:
                    st.error(f"Failed to add analyst (names must be unique): {e}")
